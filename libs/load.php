...
function require_mysql_db() {
	...
	$GLOBALS['db'] = new MySQL( DB_USER, DB_PASS, DB_NAME, DB_HOST, DB_PRE );		
}
/**
...
function site_user() {
	...
	if ( !class_exists('user') )
		require( libs_path . '/class-user.php' );
...
function loaded_component() {		
	...
	require( libs . '/theme-generals.php' );
...
function global_query(){
	...
	require_once( libs_path . '/query.php' );
	endif;
}

function global_paging(){
	if ( ! class_exists( 'Pagination' ) ):
	require_once( libs_path . '/class-pagination.php' );

	$GLOBALS['paging'] = new Pagination();
	endif;
}
...
function sidebar_default(){
	global $sidebar_default;
	$sidebar_default = array(
		"post" => array('t'	=>'Postingan','l'=>'?admin&s=posting'),
		"media" => array('t'=>'Berangkas','l'=>'?admin=single&s=media'),
		"appearance" => array('t'=>'Tampilan','l'=>'?admin&s=appearance'),
		"backup" => array('t'=>'Cadangan','l'=>'?admin&s=backup'),
		"users" => array('t'=>'Pengguna','l'=>'?admin&s=users'),
		"options" => array('t'=>'Pengaturan','l'=>'?admin&s=options')
	);
}