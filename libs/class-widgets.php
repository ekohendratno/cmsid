...
function get_sidebar_action( $i ){
	...
	if( $p && $p != 'page' ){	
		...
		$sidebar_action_cout = count( (array) $sidebar_action->$p );
	
		if( $p && $sidebar_action_cout > 0 ):
			foreach( $sidebar_action->$p as $sidebar_id => $status ){
				if( $sidebar_id == $i )
...
function get_sidebars_widgets($deprecated = true) {
	...
	global $query, $registered_widgets, $_sidebars_widgets, $sidebars_widgets;
