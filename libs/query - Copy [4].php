...
class Query{	
	...
	public function get_options( $option ){
		...
		if( $option['q'] == 'count' )
			$this->query = $db->query("SELECT COUNT(*) AS total FROM $db->options WHERE option_name='$option'");
			
		if( $option['q'] == '' )
			$this->query = $db->query("SELECT * FROM $db->options WHERE option_name='$option'");
				
		if( $db->num( $this->query ) > 0 && $obj = $db->fetch_obj( $this->query ) ):
