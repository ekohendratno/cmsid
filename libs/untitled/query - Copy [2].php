...
class Query {
	...
	public function get($name='', $urlDecode=FALSE, $san=FALSE) {
		...
		{
			if ($urlDecode && $san) {
				$this->param[$name] = $this->clean(
				$this->sanitize($_GET[$name]), 
				TRUE);
			}
			elseif ($urlDecode) {
				$this->param[$name] = $this->clean($_GET[$name], TRUE);
			}
			elseif ($san) {
				$this->param[$name] = $this->clean(
				$this->sanitize($_GET[$name]), 
				...
				$this->param[$name] = $this->clean($_GET[$name], FALSE);
			}
		} 
		else {
	...
	public function post($name='', $urlDecode=FALSE, $san=FALSE) {
		...
		if (isset($_POST[$name])) 
		{
			...
			elseif ($urlDecode) {
				$this->param[$name] = $this->clean($_POST[$name], TRUE);
			}
			elseif ($san) {
				$this->param[$name] = $this->clean(
				$this->sanitize($_POST[$name]), 
				...
				$this->param[$name] = $this->clean($_POST[$name], FALSE);
			}
		} 
		else {
			$this->param[$name] = NULL;
	 ...
	 */
	private function clean($data, $isUrlEncoded=FALSE) {
		return ($isUrlEncoded) ?
